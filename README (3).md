...
A single-page, all-in-one study dashboard for learners of every age — from primary school kids to college students. Search any topic and get a sourced summary, a related image, a link to video lessons, and a personal notebook — all on one page, with no sign-up, no backend, and no API keys. The Reference Desk on every result spans K-12 worksheets (CK-12, NCERT) all the way up to college-level textbooks (OpenStax), so the same search box works whether you're looking up "photosynthesis" for a 5th-grade worksheet or for an undergrad biology class.
...
| Reference Desk (textbooks, workbooks, worksheets) | Every result card ends with a row of links that open a live search for that exact topic on **Khan Academy**, **OpenStax**, **CK-12**, **LibreTexts**, and **NCERT** | All five are free, openly licensed education providers spanning every grade level — CK-12 and NCERT cover primary and secondary school (K-12), Khan Academy runs from grade school through early college, and OpenStax and LibreTexts cover college-level textbooks. Khan Academy uses its real public search URL (`khanacademy.org/search?page_search_query=`); the other four use a Google `site:` search scoped to that domain, since they don't expose a stable public query API — either way, the link always leads to real content on a legitimate source, never a pirated PDF |
...
- This is a static front-end only; there's no login, no server, and no shared/multi-user data — it's a personal study tool, not a classroom platform. It works equally well for a primary school kid looking up a homework topic or a college student researching a term paper — the content sources scale across grade levels automatically.
